...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 03:40:35 2026
...
def run_experiments(n_reps=N_REPS):
    ...
    rep_df.to_csv(f"policy_A_replications_R{n_reps}.csv", index=False)
    ...
    if len(waits_arr) > 0:
        ...
        ax.set_title(f"Policy A waiting-time tail (CCDF, R={n_reps})")
        ...
        fig.tight_layout()
        fig.savefig(f"fig_wait_tail_ccdf_policy_A_R{n_reps}.png", dpi=300)
        plt.close(fig)


    print("\n" + "="*45)
    print(f"🚀 POLICY A KPI SCORECARD (Post Warm-up)  |  R={n_reps}")
    print("="*45)

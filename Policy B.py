...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 03:45:22 2026
...
class BoxCarSim:
    ...
    def dispatch(self):
        ...
        while self.waiting_queue and self.idle_drivers:
            best_r_idx = -1
            best_driver = None
            min_score = float('inf')
            best_dist = 0
            ...
            for i, rider in enumerate(self.waiting_queue):
                wait_time_hr = self.env.now - rider['arr_time']
                ...
                for did, d_info in self.idle_drivers.items():
                    ...
                    score = dist - WAIT_WEIGHT * wait_time_hr
            ...
            if best_driver is not None:
                rider = self.waiting_queue.pop(best_r_idx)
                d_info = self.idle_drivers.pop(best_driver)
                rider['match_time'] = self.env.now
                self.env.process(self.serve_trip(best_driver, d_info, rider, best_dist))
            else:
                break
...
def run_experiments(n_reps=N_REPS):
    ...
    rep_df.to_csv(f"policy_B_replications_R{n_reps}.csv", index=False)
    ...
    if len(waits_arr) > 0:
        ...
        xmax = min(max(m_p95 * 2.0, np.percentile(waits_arr, 99), 12), waits_arr.max())
        ax.set_xlim(0.1, xmax)
        ax.set_xlabel("Waiting time (minutes)")
        ax.set_ylabel("Tail probability  P(wait > w)")
        ax.set_title(f"Policy B waiting-time tail (CCDF, R={n_reps})")

        ax.axvline(m_p95, linestyle="--", linewidth=1.6)
        ax.axhline(0.05, linestyle=":", linewidth=1.4)
        
        txt = (
        f"Median = {m_med:.2f} min\n"
        f"P95 = {m_p95:.2f} min\n"
        f"P(wait > P95) = 0.05"
        )
        ...
        va="top", ha="left", bbox=dict(boxstyle="round,pad=0.35", fc="white", ec="black", lw=1.0)
        )

        ax.grid(True, which="both", linestyle=":", linewidth=0.8)
        fig.tight_layout()
        fig.savefig(f"fig_wait_tail_ccdf_policy_B_R{n_reps}.png", dpi=300)
        plt.close(fig)


    print("\n" + "="*45)
    print(f"🚀 POLICY B KPI SCORECARD (Post Warm-up)  |  R={n_reps}")
    print("="*45)
